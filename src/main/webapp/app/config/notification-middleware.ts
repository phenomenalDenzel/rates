      ...
      if (action.meta && action.meta.errorMessage) {
        ...
        if (!(response.status === 401 && (error.message === '' || (data && data.path && data.path.includes('/api/account'))))) {
          ...
          switch (response.status) {
            ...
            case 400: {
              ...
              if (errorHeader) {
                ...
                for (i = 0; i < fieldErrors.length; i++) {
                  ...
                  const fieldName = translate(`ratesApp.${fieldError.objectName}.${convertedField}`);
            ...
            case 401:
              if (data.path && data.path === '/api/authentication') {
                toast.error(translate('login.messages.validate.login.invalid.credential'));
              }
              break;
            case 404:
              ...
              break;
            case 412:
              if (data.title && data.title.toLowerCase().endsWith('insufficient fund')) {
                addErrorAlert(data.title, 'error.insufficient.fund');
              }
              break;
