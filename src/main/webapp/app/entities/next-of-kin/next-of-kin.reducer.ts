...
export const ACTION_TYPES = {
  SEARCH_NEXTOFKINS: 'nextOfKin/SEARCH_NEXTOFKINS',
...
export default (state: NextOfKinState = initialState, action): NextOfKinState => {
  switch (action.type) {
    case REQUEST(ACTION_TYPES.SEARCH_NEXTOFKINS):
    ...
    case FAILURE(ACTION_TYPES.SEARCH_NEXTOFKINS):
    ...
    case SUCCESS(ACTION_TYPES.SEARCH_NEXTOFKINS):
...
const apiSearchUrl = 'api/_search/next-of-kins';

// Actions

export const getSearchEntities: ICrudSearchAction<INextOfKin> = (query, page, size, sort) => ({
  type: ACTION_TYPES.SEARCH_NEXTOFKINS,
  payload: axios.get<INextOfKin>(`${apiSearchUrl}?query=${query}`),
});
