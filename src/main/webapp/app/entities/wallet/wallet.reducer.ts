...
export const ACTION_TYPES = {
  SEARCH_WALLETS: 'wallet/SEARCH_WALLETS',
...
export default (state: WalletState = initialState, action): WalletState => {
  switch (action.type) {
    case REQUEST(ACTION_TYPES.SEARCH_WALLETS):
    ...
    case FAILURE(ACTION_TYPES.SEARCH_WALLETS):
    ...
    case SUCCESS(ACTION_TYPES.SEARCH_WALLETS):
...
const apiSearchUrl = 'api/_search/wallets';

// Actions

export const getSearchEntities: ICrudSearchAction<IWallet> = (query, page, size, sort) => ({
  type: ACTION_TYPES.SEARCH_WALLETS,
  payload: axios.get<IWallet>(`${apiSearchUrl}?query=${query}`),
});
