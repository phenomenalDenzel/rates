  ...
  return (
    <div>
      <h2 id="location-heading">
        <Translate contentKey="ratesApp.location.home.title">Locations</Translate>
        <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
          <FontAwesomeIcon icon="plus" />
          &nbsp;
          <Translate contentKey="ratesApp.location.home.createLabel">Create new Location</Translate>
        </Link>
      </h2>
      <Row>
        <Col sm="12">
            ...
            <AvGroup>
              <InputGroup>
                <AvInput
                  ...
                  placeholder={translate('ratesApp.location.home.search')}
                />
        ...
        {locationList && locationList.length > 0 ? (
            ...
            <tbody>
              {locationList.map((location, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{location.customerId ? <Link to={`customer/${location.customerId}`}>{location.customerId}</Link> : ''}</td>
                  <td>{location.localGovtName ? <Link to={`local-govt/${location.localGovtId}`}>{location.localGovtName}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="ratesApp.location.home.notFound">No Locations found</Translate>
            </div>
          )
        )}
      </div>
