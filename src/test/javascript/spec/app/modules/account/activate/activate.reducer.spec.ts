...
import activate, { ACTION_TYPES, activateAction, reset, generateOTP, validateOTP } from 'app/modules/account/activate/activate.reducer';
...
describe('Activate reducer tests', () => {
  ...
  describe('Actions', () => {
    ...
    it('dispatches ACTION_TYPES.RESET actions', async () => {
      const expectedActions = [
        {
          type: REQUEST(ACTION_TYPES.OTP_ACTIVATION),
          meta: {
            successMessage: null,
          },
        },
        {
          type: SUCCESS(ACTION_TYPES.OTP_ACTIVATION),
          payload: resolvedObject,
          meta: {
            successMessage: null,
          },
      ...
      await store.dispatch(validateOTP({}));
      expect(store.getActions()).toEqual(expectedActions);
    });
    it('dispatches ACTION_TYPES.RESET actions', async () => {
      const expectedActions = [
        {
          type: REQUEST(ACTION_TYPES.RESEND_OTP),
          meta: {
            successMessage: null,
          },
        },
        {
          type: SUCCESS(ACTION_TYPES.RESEND_OTP),
          payload: resolvedObject,
          meta: {
            successMessage: null,
          },
      ...
      await store.dispatch(generateOTP({}));
      expect(store.getActions()).toEqual(expectedActions);
    });
    it('dispatches ACTION_TYPES.RESET actions', async () => {
      const expectedActions = [
